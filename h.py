...
def smooth_scroll_to_bottom(driver):
    ...
    # スムーズなスクロールを行う
    driver.execute_script(f"window.scrollTo({{top: {page_height}, behavior: 'smooth'}});")
    # スクロールが完了するまで待機する
    time.sleep(5) # スクロールの完了を待つための時間（適宜調整）
...
def collect_image_urls(urls_filename, window):
  ...
  saved_urls = set()
  try:
    previous_position = 0  # Declare and initialize the previous_position variable
    while True:
      # スムーズなスクロールを行う
      ...
      for image in images:
        try:
          ...
          if image_url.startswith('https://pbs.twimg.com/media'):
            ...
            if high_quality_image_url not in saved_urls:
